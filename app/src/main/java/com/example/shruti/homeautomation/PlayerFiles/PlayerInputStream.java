...
/**
 * Created by Vishwas on 4/28/16.
 */
public class PlayerInputStream extends DataInputStream {
    ...
    private SeekStream stream;
    private int fullLength = 0;

    public static PlayerInputStream read(String url, File cacheDir) {
        HttpResponse res;
        DefaultHttpClient httpclient = new DefaultHttpClient();
        try {
            res = httpclient.execute(new HttpGet(URI.create(url)));
            return new PlayerInputStream(res.getEntity().getContent(), cacheDir);
    ...
    public PlayerInputStream(InputStream in, File cacheDir) {
