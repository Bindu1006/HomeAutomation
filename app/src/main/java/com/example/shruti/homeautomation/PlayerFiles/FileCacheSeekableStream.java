 ...
 */
public class FileCacheSeekableStream extends SeekStream{
     ...
     */
    public FileCacheSeekableStream(InputStream stream, File cacheDir) throws IOException {
        this.stream = stream;
        this.cacheFile = File.createTempFile("cta-cache", ".tmp", cacheDir);
    ...
    public void seek(long pos) throws IOException {
        if (pos < 0) {
            throw new IOException("FileCacheSeekableStream position < 0 ---> pos=" +pos);
        }
        pointer = pos;
    }
