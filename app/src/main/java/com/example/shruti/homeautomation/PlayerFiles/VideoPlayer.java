package com.example.shruti.homeautomation.PlayerFiles;
...
/**
 * Created by Vishwas on 4/29/16.
 */
public class VideoPlayer extends Thread implements MediaController.MediaPlayerControl {

    private static final String TAG = "MjpegPlayer";

    private SurfaceHolder mSurfaceHolder;
    private PlayerInputStream mIn = null;
    SeekStream cacheStream = null;
    ...
    public VideoPlayer(SurfaceHolder surfaceHolder, Context context) {
        mSurfaceHolder = surfaceHolder;
        mContext = context;
    }


    public VideoPlayer(SurfaceHolder holder, Context context, PlayerInputStream mIn) {
        this(holder, context);
        setSource(mIn);
    }
    ...
    private Rect destRect(int bmw, int bmh) {
        int tempx;
        int tempy;
        if (displayMode == PlayerView.SIZE_STANDARD) {
            tempx = (dispWidth / 2) - (bmw / 2);
            tempy = (dispHeight / 2) - (bmh / 2);
            return new Rect(tempx, tempy, bmw + tempx, bmh + tempy);
        }
        if (displayMode == PlayerView.SIZE_BEST_FIT) {
        ...
        if (displayMode == PlayerView.SIZE_FULLSCREEN) return new Rect(0, 0, dispWidth, dispHeight);
        return null;
    }
    ...
    public void setSource(PlayerInputStream source) {
        mIn = source;
    }
